...
- Desktop site comparison rows align a named health icon, site, three metrics, signed change and drill-down. Mobile rows align site identity and three numeric columns beneath a dark icon header; the same icons are named in the primary summary and numeric values retain accessible names. Change sits under the name. Repeated metric icons are omitted from rows.
...
- **Compact actions and copy:** repeated edit/copy controls use a 44px icon with a contextual accessible name and title; the site switcher uses the same compact control. Keep primary creation and destructive actions named. Section titles use short nouns. Client rows share one column header, retain numeric accessible labels and show a named attention icon; evidence stays available in the detail view. Primary metric icons appear once alongside their labels, never beside every table value.
