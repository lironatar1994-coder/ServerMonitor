...
- Desktop site comparison rows align identity, three metrics, change and a drill-down affordance. Mobile rows place identity first, three labeled metrics beneath, then the change note.
...
- **Comparison rows and charts:** make site rows clickable and show small-sample caveats beside the change. Chart legends distinguish the current solid series from the previous dashed series; do not depend on color alone.
- **Hints and diagnostics:** short methodology lives behind the information control or expandable diagnostics. Keep `מבקרים משוערים`, `ביקורים שנמדדו` and contact-click language precise. Browser signals and server candidates are separate measurements, not additive audiences.
