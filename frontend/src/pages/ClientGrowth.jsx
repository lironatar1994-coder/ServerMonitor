  ...
  return <div className="page page--visitors growth-workspace">
    ...
    <DataState loading={loading} error={error} onRetry={() => load()}>{data && <>
      {!id ? <>
        ...
        {tab === 'overview' && <>
          <StatRow><Stat label="סשנים שנמדדו" value={m.sessions} hint="סשנים אנונימיים במדידה החדשה בלבד; סינון אוטומציה אינו הוכחה לאדם. אין השלמה של היסטוריה שלא נמדדה." /><Stat label="סשנים עם לחיצת קשר" value={m.contact_click} /><Stat label="פניות ממערכת המקור" value={m.confirmed} hint="רשומות שנשמרו במקור המחובר. אין שיוך אוטומטי לסשן הדפדפן." /><Stat label="פניות בדיווח ידני" value={m.manual} /></StatRow>
          ...
          <Panel title="תהליך יצירת קשר" hint="השלבים הם סשנים עם כל סוג פעולה, לא משפך מסודר ולא שיעור המרה לפנייה מאומתת."><div className="growth-stages">{[['form_start', 'התחלת טופס'], ['form_submit', 'ניסיון שליחה'], ['form_error', 'שגיאת אימות'], ['form_success_observed', 'הצלחה שדווחה בדפדפן']].map(([k, label]) => <div key={k}><strong>{num(m[k])}</strong><span>{label}</span></div>)}</div></Panel>
          <div className="grid grid--1-1"><Panel title="עמודים ופעולות">{data.pages.length ? data.pages.map(p => <div className="growth-item" key={p.path}><b dir="ltr">{p.path}</b><span>{num(p.sessions)} סשנים · {num(p.contacts)} עם קשר</span></div>) : <Empty text="הנתונים יופיעו אחרי המדידות הראשונות" />}</Panel>
            <Panel title="מקורות וקמפיינים">{data.sources.length ? data.sources.map((s, i) => <div className="growth-item" key={i}><div><b>{s.source || 'ישיר / לא ידוע'}</b><small>{[s.medium, s.campaign].filter(Boolean).join(' · ')}</small></div><span>{num(s.sessions)} סשנים · {num(s.contacts)} עם קשר</span></div>) : <Empty text="טרם נמדדו מקורות הגעה" />}</Panel></div>
        </>}
        {tab === 'goals' && <>
