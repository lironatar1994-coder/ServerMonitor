...
import { DataState, Empty, Hint, IconAction, PageHead, Panel, Stat, StatRow, Tabs } from '../components/AnalyticsParts';
...
function InsightList({ items, onTask, busy }) {
  if (!items.length) return <Empty text="אין מספיק נתונים להמלצות" />;
  return <div className="growth-list">{items.map(item => <article className="growth-item" key={item.key}>
    <div><strong>{item.title}</strong><details className="insight-evidence"><summary>פרטים</summary><p>{item.evidence}</p><small>{item.action}</small></details></div>
    {onTask && <button className="btn" disabled={busy || Boolean(item.task_id)} onClick={() => onTask(item)}>{item.task_id ? 'קיימת משימה' : 'משימה'}</button>}
  </article>)}</div>;
}
function ShareDraft({ id, days, notify }) {
  ...
  return <Panel title="טיוטת סיכום" hint="נפתחת טיוטה באפליקציה שבחרת. פתיחת הטיוטה אינה אישור שההודעה נשלחה. אין ללקוח גישה למערכת.">
    <div className="growth-actions"><button className="btn btn--primary" disabled={busy} onClick={generate}>{draft ? 'טיוטה חדשה' : 'הכנת טיוטה'}</button><span className="muted">לבדיקה לפני שיתוף</span></div>
    {draft && <div className="form growth-draft">
      ...
      <div className="growth-actions">
        ...
        <a className="btn" href={`mailto:?subject=${encodeURIComponent(draft.subject)}&body=${encodeURIComponent(draft.body)}`}><Mail />מייל</a>
        <a className="btn" href={`https://wa.me/?text=${encodeURIComponent(draft.body)}`} target="_blank" rel="noopener noreferrer"><MessageCircle />WhatsApp</a>
      </div>
...
export default function ClientGrowth() {
  ...
  return <div className="page page--visitors growth-workspace">
    <PageHead title={id ? data?.app?.name || 'לקוח' : 'לקוחות'} meta={id ? <Link className="crumb" to="/clients">לקוחות</Link> : null}>
    ...
    <DataState loading={loading} error={error} onRetry={() => load()}>{data && <>
      {!id ? <>
        ...
        <Panel title="מעקב" hint="הסדר מתעדף פניות שממתינות, בעיות איסוף ומשימות באיחור. ההשוואה היא של כל אתר לעצמו.">
          <div className="growth-toolbar"><label className="growth-search"><span className="sr-only">חיפוש לקוח</span><input placeholder="חיפוש לקוח" value={search} onChange={e => setSearch(e.target.value)} /></label><Tabs tabs={[{ id: 'all', label: 'לקוחות' }, { id: 'attention', label: 'לטיפול' }]} value={filter} onChange={setFilter} /></div>
          <div className="growth-client-table">
          <div className="growth-client-head" aria-hidden="true"><span>לקוח</span><span>פניות</span><span>פתוחות</span><span>משימות</span><span /></div>
          <div className="growth-list">{sites.map(site => <Link className="growth-client" key={site.id} to={`/clients/${site.id}`}>
            ...
            <dl><div><dt className="sr-only">פניות בתקופה</dt><dd>{num(site.metrics.lead_received)}</dd></div><div><dt className="sr-only">פניות פתוחות</dt><dd>{num(site.open_leads)}</dd></div><div><dt className="sr-only">משימות פתוחות</dt><dd>{num(site.open_tasks)}</dd></div></dl>
            <span className="growth-row-state">{site.insights[0]?.title ? <CircleAlert role="img" className="is-attention" aria-label={site.insights[0].title} title={site.insights[0].title} /> : <ArrowUpRight aria-hidden="true" />}</span>
          </Link>)}</div></div>{!sites.length && <Empty text="אין לקוחות בסינון" />}
        </Panel>
      </> : <>
        ...
        {tab === 'overview' && <>
          ...
          <Panel title="לטיפול היום" hint="פניות ומשימות מכל התקופות. לחיצות בדפדפן אינן פניות.">
            {queue.slice(0, 8).map(row => <div className="growth-item" key={`${row.kind}:${row.id}`}><div><strong>{row.title}</strong><small>{row.kind === 'leads' ? origins[row.origin] || 'מקור מחובר' : 'משימה'} · {row.due_at ? formatDateTime(row.due_at) : 'ללא מועד'}</small></div><span className={isOverdue(row) ? 'is-attention' : 'muted'}>{isOverdue(row) ? 'באיחור' : row.kind === 'leads' ? leadStatuses[row.status] : taskStatuses[row.status]}</span><IconAction icon={Pencil} label={`עדכון ${row.title}`} onClick={() => { setTab(row.kind); startEdit(row.kind, row); }} /></div>)}
            {!queue.length && <Empty text="אין פניות או משימות פתוחות" />}
            {queue.length > 8 && <button className="text-action" onClick={() => setTab('tasks')}>כל המשימות</button>}
          </Panel>
          <details className="measurement-details"><summary>מדידה ותובנות</summary>
          {data.app.name === 'LA webs' && <Panel title="ביקור פנימי" hint="ההחרגה חלה על הדפדפן שבו נפתח הקישור, לביקורים עתידיים בלבד. ביקורים קודמים אינם מסווגים מחדש.">
            <div className="growth-actions"><a className="btn" href="https://lawebs.co.il/?monitor_internal=1" target="_blank" rel="noopener noreferrer">ביקור ללא ספירה</a><a className="btn" href="https://lawebs.co.il/?monitor_internal=0" target="_blank" rel="noopener noreferrer">חידוש ספירה</a></div>
          </Panel>}
          ...
          <div className="grid grid--1-1"><Panel title="עמודים ופעולות">{data.pages.length ? data.pages.map(p => <div className="growth-item" key={p.path}><b dir="ltr">{p.path}</b><span>{num(p.sessions)} ביקורים · {num(p.contacts)} עם קשר</span></div>) : <Empty text="טרם נמדדה פעילות" />}</Panel>
        ...
        {tab === 'activity' && <Panel title="היסטוריה"><div className="growth-list">{data.activity.map(a => <div className="growth-item" key={a.id}><div><strong>{a.summary}</strong><small>{a.actor}</small></div><time>{formatDateTime(a.occurred_at)}</time></div>)}</div>{!data.activity.length && <Empty text="אין עדכונים" />}</Panel>}
