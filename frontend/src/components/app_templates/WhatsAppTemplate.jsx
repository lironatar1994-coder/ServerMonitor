...
const WhatsAppTemplate = ({ app }) => {
  ...
  return (
    <div className="whatsapp-workspace">
      ...
      <section className="whatsapp-grid"><form className="whatsapp-test" onSubmit={handleSend}><MessageCircle /><span className="eyebrow">בדיקה</span><h2>שליחת הודעה</h2>{feedback && <div className="banner banner--success">{feedback}</div>}<label>מספר יעד<input dir="ltr" value={phone} onChange={(event) => setPhone(event.target.value)} required /></label><label>הודעה<textarea rows="4" value={message} onChange={(event) => setMessage(event.target.value)} required /></label><button type="submit" disabled={sending}><Send /> {sending ? 'שולח…' : 'שליחה'}</button></form><div className="whatsapp-log"><LiveTerminal appId={app.id} /></div></section>
    </div>
  );
};
