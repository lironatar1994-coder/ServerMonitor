...
- `/apps/server-stats` reports Linux `MemAvailable`-based RAM use, swap details, root-filesystem usage, load, and top processes. Its `resources` payload attributes all descendant processes to every running PM2 app, enriching database-mapped apps with their display names, and ranks memory-heavy processes.
- Storage visibility must scan only the production paths declared in `resourceUsage.js`, run `du` without blocking the Node event loop, cache snapshots for five minutes, refresh stale data in the background, distinguish dependency, rollback, backup, log, and cache bytes, and return stale cached data when a refresh fails.
